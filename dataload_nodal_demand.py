# -*- coding: utf-8 -*-
"""
Created on Tue Jun  5 15:20:10 2018
...
import os
import pyproj
# os.environ["PROJ_LIB"] = "C:\\Anaconda3\\Library\\share"
...
def nodal_demand(ddir, countries, demand_el, nodes):
    # ddir, countries, demand_el, nodes = self.ddir, self.settings["countries"], demand, self.nodes
    ...
    for node in [n for n in nodes.index if n not in nodal_demand.columns]:
        nodal_demand[node] = 0
...
def only_nuts3_zones(ddir):
    ...
    nuts3 = pd.read_csv(ddir.joinpath('demand_node_data/NUTS_AT_2013.csv'),
...
def standard_load_profiles(ddir):
    ...
    slp_h0_60min = pd.read_excel(ddir.joinpath('demand_node_data/bdew-slp-H0-2015.xls'),
    ...
    slp_g0_60min = pd.read_excel(ddir.joinpath('demand_node_data/bdew-slp-G0-2015.xls'),
...
def read_comsumption_data(ddir):
    consumtion = pd.read_csv(ddir.joinpath('demand_node_data/nrg_105a_1_Data.csv'),
                             encoding = "ISO-8859-1", usecols=["GEO",'INDIC_NRG', 'Value'])
    consumtion.Value = pd.to_numeric(consumtion.Value.str.replace(".",""), errors='coerce').fillna(0)
...
def load_nuts_data(ddir):
    ...
    pop_nuts3 = pd.read_csv(ddir.joinpath('demand_node_data/nama_10r_3popgdp_1_Data.csv'),
    ...
    gva_nuts3 = pd.read_csv(ddir.joinpath('demand_node_data/nama_10r_3gva_1_Data.csv'),
...
def get_nodal_demand(ddir, country, demand, nodes, scaling=None):
    ...
    # country = "DK"
    # nodes = nodes_tmp
    # ddir = Path('C:/Users/riw/tubCloud/Uni/ModelingData/data')
    
    # ddir = data.ddir
    # nodes = data.nodes
    # scaling = {"household": 0.3,
    #             "commercial": 0.3,
    #             "industry": 0.4,
    #             "demand": 1.5}
    ...
    # self = data
    # country = "DE"
    # timeseries = load_timeseries_data(self.ddir, self.settings["zones"], self.time_horizons["full"])
    # demand = process_demand(timeseries, self.settings["zones"],
    #                         self.time_horizons["demand_el"], self.settings["demand_scaling"])
    # nodes = data.nodes
    # scaling = 1
    ...
    if isinstance(scaling, dict):
        ...
        print(f"Splits between Sectors from Input: Household " + hh + ", " + com + ", " + ind)
    ...
    nuts_rg_01m_2013 = gpd.read_file(str(ddir.joinpath('demand_node_data/NUTS_2013_01M_SH/data/NUTS_RG_01M_2013.shp')))
    ...
    crs = 'epsg:4326'
    node_data = gpd.GeoDataFrame(node_data, crs=crs, geometry=geometry)
    ...
    nuts_no_node = node_in_nuts['index_right'].isnull().loc[lambda df: df == True].index
    nuts_centroids = nuts_rg_01m_2013.centroid
    nuts_no_node = nuts_centroids.loc[nuts_no_node]
